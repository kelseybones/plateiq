// Shared primitives: ring, bars, status bar, tabbar, helpers
...
const Ring = ({ value, goal, size = 220, stroke = 16, label, sublabel, accentVar = "--accent", animate = true }) => {
  const radius = (size - stroke) / 2;
  const circ = 2 * Math.PI * radius;
  const pct = Math.min(1, value / goal);
  const target = circ * (1 - pct);

  const [draw, setDraw] = React.useState(animate ? circ : target);
  React.useEffect(() => {
...
const ProgressBar = ({ value, goal, accent = "var(--accent)", height = 10, animate = true }) => {
...
// Bottom tab bar
const TabBar = ({ active, onChange, onPlus }) => {
  const tabs = [
    { id: "home", icon: "home", label: "Home" },
    { id: "log", icon: "search", label: "Log" },
  ...
  return (
    <div className="tabbar">
      <div className="tabbar-inner">
        ...
        <button className="tab-fab" onClick={onPlus} aria-label="Quick log">
...
const Ticker = ({ value, duration = 900, format = (v) => Math.round(v).toLocaleString() }) => {
...
Object.assign(window, { StatusBar, Ring, ProgressBar, TabBar, Ticker, Chip });
