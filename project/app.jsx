// Main App shell — phone with state, routing, onboarding gate, tweaks
...
const App = () => {
  ...
  React.useEffect(() => {
    ...
    root.style.setProperty("--accent-ink", lum > 0.6 ? "#0a0b0d" : "#ffffff");
    root.style.setProperty("--m-protein", tw.accent);
  }, [tw.accent, tw.theme]);

  const [onboarded, setOnboarded] = React.useState(false);
  const [profile, setProfile] = React.useState({
    goal: "muscle", calorieGoal: 2800, pGoal: 180, cGoal: 320, fGoal: 90,
  });
  const [meals, setMeals] = React.useState(SEED_MEALS);
  const [tab, setTab] = React.useState("home");

  // Routing within the home stack
  // route can be: { name: 'home' } | { name: 'mealDetail', slot } | { name: 'log', slot, custom?, quick?, editing? }
  const [route, setRoute] = React.useState({ name: "home" });
  const [mealSlot, setMealSlot] = React.useState("lunch");
  const [logView, setLogView] = React.useState("list");

  React.useEffect(() => {
    if (tw.skipOnboarding) setOnboarded(true);
  }, [tw.skipOnboarding]);
  ...
  const handleAddMeal = (entry) => {
    setMeals(m => [...m, entry]);
    // Return to whichever screen we came from (meal detail if it was opened, else home)
  ...
  // Back from food log: return to mealDetail if we came from there, else home
  const backFromLog = () => {
    if (route.fromMealDetail) setRoute({ name: "mealDetail", slot: route.slot || mealSlot });
    else setRoute({ name: "home" });
  };

  const handleOnboard = ({ goal, calories, protein, carbs, fat }) => {
    setProfile({
      goal,
  ...
  return (
    <div style={{
      ...
      <BackdropDots/>
      <a href="PlateIQ Canvas.html" style={{
        position: "fixed", top: 16, right: 16, zIndex: 50,
        padding: "10px 14px", borderRadius: 999,
        background: "var(--card)", border: "1px solid var(--border)",
        color: "var(--text)", fontFamily: "var(--f-mono)", fontSize: 11,
        letterSpacing: "0.12em", textTransform: "uppercase", textDecoration: "none",
        display: "flex", alignItems: "center", gap: 8,
      }}>
        <Icon name="chart" size={14}/> All screens
      </a>
          ...
          <>
            ...
            {logging ? (
              ...
              <div className="phone-body">
                {tab === "home" && (
                  <Dashboard data={data} layout={tw.dashLayout}
                    onLog={() => openLog()}
                    onMeal={(m) => editEntry(m, false)}
                    view={logView} onView={setLogView}
                    onOpenMeal={openMealDetail}
                  />
                )}
                {tab === "log" && <FoodLogStub onLog={() => openLog()}/>}
                {tab === "macros" && <MacroBreakdown data={data} variant={tw.macroChart}/>}
                {tab === "progress" && <Progress data={data}/>}
                {tab === "profile" && <Settings data={profile} onUpdate={setProfile}/>}
              </div>
            )}
            {showTabbar && (
              <TabBar active={tab} onChange={(t) => { setTab(t); setRoute({ name: "home" }); }} onPlus={() => openLog()}/>
            )}
          </>
        )}
      </div>
...
const FoodLogStub = ({ onLog }) => {
  React.useEffect(() => {
    const t = setTimeout(onLog, 50);
...
const PhoneCaption = ({ tab, logging, inMealDetail, onboarded }) => {
  const labels = {
    home: "Dashboard · ring-first / split / stacked",
    log: "Food logging",
...
const TweaksUI = () => {
  const ACCENTS = ["#C6FF3A", "#3B82F6", "#FF5A1F", "#FF2D55", "#A78BFA"];
  return (
    <TweaksPanel title="PlateIQ tweaks">
      <TweakSection label="Theme">
        <TweakRadio tweakKey="theme" options={[["dark", "Dark"], ["light", "Light"]]}/>
      </TweakSection>
      <TweakSection label="Accent color">
        <TweakColor tweakKey="accent" options={ACCENTS}/>
      </TweakSection>
      <TweakSection label="Dashboard layout">
        <TweakRadio tweakKey="dashLayout" options={[["ring", "Ring"], ["split", "Split"], ["stacked", "Stacked"]]}/>
      </TweakSection>
      <TweakSection label="Macro chart">
        <TweakRadio tweakKey="macroChart" options={[["donut", "Donut"], ["bar", "Bar"], ["radial", "Radial"]]}/>
      </TweakSection>
      <TweakSection label="Onboarding">
        <TweakToggle tweakKey="skipOnboarding" label="Skip to dashboard"/>
      </TweakSection>
    </TweaksPanel>
  );
};
