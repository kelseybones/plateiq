// Food logging — search, recents, detail/edit, scan, custom, quick-add
...
const FoodLog = ({ onClose, onAdd, onUpdate, mealSlot, setMealSlot, editing, custom: showCustom, quick: showQuick }) => {
  ...
  const [selected, setSelected] = React.useState(editing ? { entry: editing, food: editing._food } : null);
  const [scanning, setScanning] = React.useState(false);
  const [customMode, setCustomMode] = React.useState(showCustom || false);
  const [quickMode, setQuickMode] = React.useState(showQuick || false);
  ...
  if (customMode) return <CustomFood onBack={() => setCustomMode(false)} onSave={(entry) => { onAdd(entry); setCustomMode(false); }} mealSlot={mealSlot}/>;
  if (quickMode) return <QuickAdd onBack={() => setQuickMode(false)} onSave={(entry) => { onAdd(entry); setQuickMode(false); }} mealSlot={mealSlot}/>;
...
// Detail view — grams-based, prefilled with servingG
const FoodDetail = ({ food, entry, mealSlot, onBack, onLog }) => {
  const editing = !!entry;
  const initialGrams = editing ? entry.grams : food.servingG;
  const [grams, setGrams] = React.useState(String(initialGrams));
  const g = +grams || 0;
...
// Custom food — user enters everything, including grams
...
Object.assign(window, { FoodLog, FoodDetail });
